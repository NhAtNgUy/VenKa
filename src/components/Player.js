...
const Player = ({ videoId }) => {
  ...
  const [isScoringEnabled, setIsScoringEnabled] = useState(true); // Thêm trạng thái bật/tắt chấm điểm
  ...
  const startRecording = useCallback(async () => {
    try {
      ...
      mediaRecorderRef.current.onstop = async () => {
        ...
        // Gửi file âm thanh đến backend chỉ khi chấm điểm được bật
        if (isScoringEnabled) {
  ...
  // Tự động bắt đầu/dừng ghi âm khi trạng thái phát thay đổi
  useEffect(() => {
  ...
  // Early return nếu không có videoId
